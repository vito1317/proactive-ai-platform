...
class SkillRunner
{
    ...
    private function decide(Conversation $conv, string $message, array $obs, bool $forceTool = false, ?callable $onThought = null, string $planNote = ''): ?array
    {
        ...
        - **【訂票/訂房/排行程/比價/在特定網站操作 → 一律用瀏覽器實際操作，不要只靠 web-search 看文字】**：
          ...
          **【手機內建瀏覽器的 Google Maps 地圖本體可能空白（WebGL 渲染限制）】**：若要的是「車程時間/距離/路線建議」這種【資料】，
          優先用 Google 搜尋（讀 AI 摘要，例如直接搜「A 到 B 開車多久」）拿到答案，不要卡在空白的地圖畫面；
          只有使用者明確要「在地圖上實際操作/看地圖」時才堅持用 Maps，且先 browser_reload 試著讓地圖載出來。
